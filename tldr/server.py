#!/usr/bin/env python3
"""
TLDR Brain Server - Flask API for code visualization.

This module provides a Flask server for serving the brain visualization UI
and APIs for querying the project data.

Usage:
    CLI:
        tldr brain serve [path] [--port PORT]
        tldr brain serve .
        tldr brain serve /path/to/project --no-build
    
    Python:
        from tldr.server import create_app
        app = create_app(project_root=Path('/path/to/project'))
        app.run(port=5000)

The server looks for brain.json in:
    1. {project}/.tldr/brain.json (default location)
    2. {project}/brain.json (backwards compatibility)
"""
...
def create_app(project_root: Path = None, brain_file: Path = None):
    ...
    Args:
        ...
        brain_file: Path to brain.json (default: project_root/.tldr/brain.json,
                    falls back to project_root/brain.json)
    ...
    if brain_file is None:
        # Try .tldr/brain.json first (new default location)
        tldr_brain = project_root / ".tldr" / "brain.json"
        root_brain = project_root / "brain.json"
        
        if tldr_brain.exists():
            brain_file = tldr_brain
        elif root_brain.exists():
            brain_file = root_brain  # Backwards compatibility
        else:
            # Default to .tldr/brain.json even if it doesn't exist yet
            brain_file = tldr_brain
...
app = None
PROJECT_ROOT = Path.cwd()
# Check .tldr/brain.json first, then fall back to root
BRAIN_FILE = PROJECT_ROOT / ".tldr" / "brain.json"
if not BRAIN_FILE.exists() and (PROJECT_ROOT / "brain.json").exists():
